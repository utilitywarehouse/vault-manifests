...
# This script generate the CA certificate+key and server certificate+key on a
# start only if there is no existing cert or its expired
...
# Exit if certificate exits and its not expiring soon
if [[ -f "${ca_crt}" ]]; then
    now_seconds=$(date +%s)
    ...
    if [[ ${validity} -gt 300 ]]; then # 5m
        echo "certificate found with validity:${validity}s exiting"
        exit 0
    fi
fi
